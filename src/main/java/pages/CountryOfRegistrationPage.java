...
public class CountryOfRegistrationPage extends BasePage {
    ...
    public List<String> eliminateUnwantedButtonsDisplayed(List<String> list) {
        return list.stream()
                ...
                .filter(line -> !"EU vehicle category M2 checkmark".equals(line))
                ...
                .filter(line -> !"English (Japan)".equals(line))
                .filter(line -> !"Deutsch (Deutschland)".equals(line))
                .filter(line -> !"Norsk".equals(line))
